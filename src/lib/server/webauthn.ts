...
export function deleteUserPasskeyCredential(userId: number, credentialId: Uint8Array): boolean {
	const result = db.execute('DELETE FROM passkey_credential WHERE id = ? AND user_id = ?', [
		credentialId,
		userId
	])
...
export function deleteUserSecurityKeyCredential(userId: number, credentialId: Uint8Array): boolean {
	const result = db.execute('DELETE FROM security_key_credential WHERE id = ? AND user_id = ?', [
		credentialId,
		userId
	])
	return result.changes > 0
